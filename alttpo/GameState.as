...
class GameState {
  ...
  bool deserialize(array<uint8> r, int c) {
    ...
    while (c < maxc) {
      ...
      switch (packetType) {
        ...
        case 0x13: c = deserialize_overlord_u8(r, c); break;
        case 0x14: c = deserialize_overlord_u16(r, c); break;
        case 0x15: c = deserialize_enemy_data(r, c); break;
        case 0x16: c = deserialize_enemy_segment_data(r, c); break;
        default:
  ...
  int deserialize_overlord_u8(array<uint8> r, int c) {
    uint8 mask = r[c++];
    for (uint s = 0; s < 8; s++) {
      if ((mask & (1 << s)) == 0) {
  ...
  int deserialize_overlord_u16(array<uint8> r, int c) {
    uint8 mask = r[c++];
    for (uint s = 0; s < 8; s++) {
      if ((mask & (1 << s)) == 0) {
        overlordU16Data[(ol16_scr << 4) + (s << 1)] = 0;
        overlordU16Data[(ol16_scr << 4) + (s << 1) + 1] = 0;
        continue;
      }
